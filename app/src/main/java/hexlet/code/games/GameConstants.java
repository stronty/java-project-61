package hexlet.code.games;

public class GameConstants {
    public static final int STANDARD_RANGE = 100;

    public static final int SEQ_STEP_RANGE = 10;
    public static final int SEQ_MIN_LENGTH = 5;
    public static final int SEQ_MAX_LENGTH = 10;

    public static final int TOTAL_ROUNDS = 3;

    public static final int PRIME_RANGE = 500;
    public static final int PRIME_NUMBERS_START = 3;

    public static final int TOTAL_OPERATORS = 3;
}
